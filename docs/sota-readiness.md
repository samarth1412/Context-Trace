...
- Failure macro-F1: `0.367`
- Root-cause accuracy: `0.480`
...
- `no_failure_detected -> answer_overreach`: 63 cases. The verifier is
...
- The semantic verifier now handles common news-summary paraphrases, generated
  summary prefixes, QA boilerplate/list markers, relation/appositive evidence
  variants, and structured JSON evidence for clear attributes such as Wi-Fi,
  ...
  failure macro-F1 `0.270` to `0.367` and root-cause accuracy `0.360` to
  `0.480` while holding dangerous false-green rate at `0.005`.
- The current semantic verifier scores failure macro-F1 `0.367`, root-cause
  accuracy `0.480`, dangerous false-green rate `0.005`, and evidence span
...
- RAGTruth assisted review pilot, ContextTrace semantic verifier: 200 official
  ...
  rows with source evidence spans, failure macro-F1 `0.367`, root-cause
  accuracy `0.480`, citation error F1 `1.000`, evidence span overlap `0.551`,
