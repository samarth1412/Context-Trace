...
CALIBRATION = json.loads(
    (ROOT / "benchmarks/contexttrace_unseen_v1/calibration/registry.json").read_text(
        encoding="utf-8"
    )
)
...
def test_acquisition_gate_requires_external_exposure_attestation() -> None:
    ...
    catalog["calibration_registry"].pop("external_human_exposure_attestation", None)
...
def test_calibration_family_overlap_fails_closed() -> None:
    catalog = copy.deepcopy(CATALOG)
    catalog["entries"][0]["source_family"] = CALIBRATION["sources"][0]["source_family"]
...
def test_apache_release_archive_requires_published_sha512() -> None:
    catalog = copy.deepcopy(CATALOG)
    hbase = next(
        entry for entry in catalog["entries"] if entry["family_id"] == "ops_hbase"
    )
    hbase["archive_sha512"] = "0" * 64
    with pytest.raises(CatalogError, match="Apache release archive"):
        validate_catalog(catalog, CALIBRATION)
