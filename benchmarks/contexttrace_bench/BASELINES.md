...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| ContextTrace semantic verifier on RAGTruth stratified test sample | 200 | 76 | 0.343 | 0.450 | 0.005 | 1.000 | 0.551 |
...
and structured JSON evidence attributes such as Wi-Fi, reservations, parking,
ambience flags, categories, ratings, and explicit hours ranges. RAGTruth rows
use answer-level verdict scope by default; claim-count metrics apply only to
rows with explicit reviewer taxonomy overrides. The OpenAI judge is
