...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| ContextTrace semantic verifier on RAGTruth stratified test sample | 200 | 76 | 0.367 | 0.480 | 0.005 | 1.000 | 0.551 |
...
paraphrases, generated summary prefixes, relation/appositive evidence variants,
QA boilerplate/list markers, and structured JSON evidence attributes such as
Wi-Fi, reservations, parking, ambience flags, categories, ratings, hours ranges,
and day-specific schedules. RAGTruth rows use answer-level verdict scope by
default; claim-count metrics apply only to rows with explicit reviewer taxonomy
overrides. The OpenAI judge is
